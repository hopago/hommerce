...
type UseParentCategoryMutationProps = {
  bookId: string;
};

export const useParentCategoryMutation = ({
  bookId,
}: UseParentCategoryMutationProps) => {
  ...
  const mutateBookParentCategory = (parentCategory: BookParentCategory) => {
    mutate({ parentCategory });
  };

  return {
    mutateBookParentCategory,
    isPending,
  };
};
